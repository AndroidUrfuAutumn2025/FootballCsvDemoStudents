    ...
    fun parsePlayers(filePath: String): List<Player> {
        ...
        return lines.drop(1).map { line ->
            ...
            Player(
                ...
                team = parts[1].trim(),
                city = parts[2].trim(),
                position = Position.fromString(parts[3].trim()) ?: throw IllegalArgumentException("Неизвестная позиция: ${parts[3].trim()}"),
                nationality = parts[4].trim(),
                agency = if (parts[5].trim().isEmpty()) null else parts[5].trim(),
